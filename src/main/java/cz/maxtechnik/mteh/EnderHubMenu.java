...
public class EnderHubMenu extends AbstractContainerMenu {
	...
	public static final TagKey<Item> WORKSTATIONS_CRAFTING_TABLES = ItemTags.create(ResourceLocation.fromNamespaceAndPath("c", "player_workstations/crafting_tables"));
	public static final TagKey<Item> C_CRAFTING_TABLES = ItemTags.create(ResourceLocation.fromNamespaceAndPath("c", "crafting_tables"));
	...
	public EnderHubMenu(int containerId, Inventory playerInventory, Container enderChest) {
		...
		// Player Inventory (3x9) - Index 27 - 53
		for (int row = 0; row < 3; ++row) {
			for (int col = 0; col < 9; ++col) {
		...
		// Player Hotbar (1x9) - Index 54 - 62
		for (int col = 0; col < 9; ++col) {
			this.addSlot(new Slot(playerInventory, col, 44 + col * 18, 143));
		}

		// Armor Slots - Index 63 - 66 (Head, Chest, Legs, Feet)
		for (int i = 0; i < 4; ++i) {
		...
		for (int r = 0; r < 3; ++r) {
			for (int c = 0; c < 3; ++c) {
				final boolean isExtra = (r == 2 || c == 2); // 5 outer slots
	...
	public boolean hasCraftingTable() {
		// Only counts if it's in a proper inventory slot or ender chest
		// Holding on cursor or placing in crafting grid does NOT count
	...
	private static boolean isCraftingTable(ItemStack itemStack) {
		if (itemStack == null || itemStack.isEmpty()) return false;
		return itemStack.is(Items.CRAFTING_TABLE)
				|| itemStack.is(WORKSTATIONS_CRAFTING_TABLES)
				|| itemStack.is(C_CRAFTING_TABLES);
	}
	...
	public void slotsChanged(@NotNull Container container) {
		...
		if (!level.isClientSide) {
			// On Crafting Lost: return extra 3x3 items to inventory safely
	...
	public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
		...
		if (slot.hasItem()) {
			...
			// 1. Crafting Result (68) -> Inventory/Hotbar
			if (index == 68) {
			...
			// 2. Ender Chest (0 - 26) -> Inventory/Hotbar
			else if (index < 27) {
				if (!this.moveItemStackTo(slotStack, 27, 63, false)) {
					return ItemStack.EMPTY;
				}
			}
			// 3. Inventory & Hotbar (27 - 62)
			else if (index < 63) {
				EquipmentSlot equipSlot = player.getEquipmentSlotForItem(slotStack);
				// 3a. Equippable humanoid armor -> try equipping into matching armor slot (63 - 66)
				if (equipSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR) {
					int armorSlotIdx = 63 + (3 - equipSlot.getIndex());
					if (!this.moveItemStackTo(slotStack, armorSlotIdx, armorSlotIdx + 1, false)) {
						if (!this.moveItemStackTo(slotStack, 0, 27, false)) {
							if (index < 54) {
								if (!this.moveItemStackTo(slotStack, 54, 63, false)) return ItemStack.EMPTY;
							} else {
								if (!this.moveItemStackTo(slotStack, 27, 54, false)) return ItemStack.EMPTY;
							}
						}
					}
				}
				// 3b. Offhand equippable (e.g. Shield) -> try equipping into offhand slot (67)
				else if (equipSlot == EquipmentSlot.OFFHAND && !this.slots.get(67).hasItem()) {
					if (!this.moveItemStackTo(slotStack, 67, 68, false)) {
						if (!this.moveItemStackTo(slotStack, 0, 27, false)) {
							if (index < 54) {
								if (!this.moveItemStackTo(slotStack, 54, 63, false)) return ItemStack.EMPTY;
							} else {
				...
				// 3c. Regular item -> move to Ender Chest (0 - 27) or swap between main inv and hotbar
			...
			// 4. Armor (63 - 66), Offhand (67), Crafting Grid (69 - 77) -> Inventory/Hotbar or Ender Chest
			else {
				if (!this.moveItemStackTo(slotStack, 27, 63, false)) {
					if (!this.moveItemStackTo(slotStack, 0, 27, false)) {
						return ItemStack.EMPTY;
